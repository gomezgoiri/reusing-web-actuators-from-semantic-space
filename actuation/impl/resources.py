...
class LightResource(object):
    ...
    def __init__(self, desc_get_fp, desc_post_fp, ret_get_tpl_fp, out_fp, init_val = 0):
    ...
    def get(self):
        idnum = random.randint(1, 10) # super-silly id generation
        return FileUtils.generate_file_replacing(["{{id}}", "{{value}}"], [str(idnum), str(self._val)], self._ret_get_tpl_fp, self._out_file_path)
    ...
    def options(self):
        return [self._desc_get_fp, self._desc_post_fp]